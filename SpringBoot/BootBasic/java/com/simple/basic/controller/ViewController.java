...
public class ViewController {
	...
	public String quiz(Model model) {
		SimpleVO vo = SimpleVO.builder().first("박").last("수정").sno(18).regdate(LocalDateTime.now()).build();
		model.addAttribute("vo", vo);
	...
	public String quiz_result01(@RequestParam("sno") int sno,
			  					...
			  					Model model) {
		SimpleVO vo = SimpleVO.builder().first(first).last(last).sno(sno).regdate(LocalDateTime.now()).build();
		model.addAttribute("vo", vo);
